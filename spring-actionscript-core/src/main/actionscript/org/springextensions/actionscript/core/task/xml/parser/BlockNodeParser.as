		...
		protected function addConstructorArgs(builder:ObjectDefinitionBuilder, node:XML):void {
			...
			switch(nodeName) {
				case TaskNamespaceHandler.FOR_ELEMENT:
					ref = node.descendants(TaskNamespaceHandler.COUNT_PROVIDER_ELEMENT)[0].descendants(TaskNamespaceHandler.REF_ATTR)[0].text();
					break;
				case TaskNamespaceHandler.WHILE_ELEMENT:
				case TaskNamespaceHandler.IF_ELEMENT:
					ref = node.descendants(TaskNamespaceHandler.CONDITION_ELEMENT)[0].descendants(TaskNamespaceHandler.REF_ATTR)[0].text();
					break;
				break;
			}
			if (ref != null) {
