...
package org.springextensions.actionscript.core.event {
	...
	[Deprecated(message = "The static EventBus is deprecated in favor of a non-static injectable alternative in as3commons-eventbus", replacement = "org.as3commons.eventbus.impl.EventBus", since = "v1.2")]
	/**
	...
	public final class EventBus {
		...
		public static function dispatchEvent(event:Event):void {
			...
			if (eventListenerProxies != null) {
				for (i = 0; i < eventListenerProxies.length; i++) {
				...
				if (eventClass === cls) {
					...
					if (proxies != null) {
						for (i = 0; i < proxies.length; i++) {
							proxy = proxies.get(i) as MethodInvoker;
							if (proxy != null) {
								proxy.arguments = [event];
								proxy.invoke();
