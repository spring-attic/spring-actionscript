...
package org.springextensions.actionscript.ioc.autowire {
	...
	public class DefaultAutowireProcessor implements IAutowireProcessor, IObjectFactoryAware {
		...
		private static var logger:ILogger = LoggerFactory.getClassLogger(DefaultAutowireProcessor);
					...
					if ((field.hasMetaData(AUTOWIRED_ANNOTATION)) || (field.hasMetaData(INJECT_ANNOTATION))) {
						try {
							...
							if (isFieldAutowireRequired(field)) {
								logger.error("Error while autowiring property {1} of object {0}: " + err.message, field.name, objectName);
								throw err;
							}
		...
		private function isFieldAutowireRequired(field:Field):Boolean {
			...
			for each (var metadata:MetaData in fieldMetadataArray) {
				if (!metadata.hasArgumentWithKey(AUTOWIRED_ARGUMENT_REQUIRED) || metadata.getArgument(AUTOWIRED_ARGUMENT_REQUIRED).value === 'true')
					return true;
			}
			return false;
		}
		...
		protected function autoWireField(object:Object, field:Field, objectName:String):void {
			...
			for each (var metadata:MetaData in fieldMetaData) {
				if (((metadata.hasArgumentWithKey(AUTOWIRED_ARGUMENT_MODE) && metadata.getArgument(AUTOWIRED_ARGUMENT_MODE).value == AutowireMode.BYNAME.name))
					|| (metadata.hasArgumentWithKey(AUTOWIRED_ARGUMENT_NAME) || metadata.hasArgumentWithKey(""))) {
		...
		protected function findAutowireCandidateNames(clazz:Class):Array {
			...
			for (var objectName:String in _objectFactory.objectDefinitions) {
				...
				if (ClassUtils.isImplementationOf(objectClass, IFactoryObject, _objectFactory.applicationDomain)) {
					var factoryObject:IFactoryObject = _objectFactory.getObject("&" + objectName);
					objectClass = factoryObject.getObjectType();
				}

				if (objectDefinition.isAutoWireCandidate) {
					if ((objectClass === clazz) || ClassUtils.isSubclassOf(objectClass, clazz, _objectFactory.applicationDomain) || ClassUtils.isImplementationOf(objectClass, clazz, _objectFactory.applicationDomain) || isFactoryObjectForClass(objectClass, objectName, clazz)) {
			...
			for each (var explicitSingletonName:String in _objectFactory.explicitSingletonNames) {
				...
				if ((objectClass === clazz) || ClassUtils.isSubclassOf(objectClass, clazz, _objectFactory.applicationDomain) || ClassUtils.isImplementationOf(objectClass, clazz, _objectFactory.applicationDomain) || isFactoryObjectForClass(objectClass, explicitSingletonName, clazz)) {
