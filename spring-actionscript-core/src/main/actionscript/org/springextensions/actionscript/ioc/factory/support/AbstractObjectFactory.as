...
package org.springextensions.actionscript.ioc.factory.support {
	...
	import org.springextensions.actionscript.core.event.EventBusAwareObjectPostProcessor;
	import org.springextensions.actionscript.core.event.adapter.EventBusAdapter;
	...
	/**
	 * Dispatched after an object has been wired by the current object factory
	 * @eventType org.springextensions.actionscript.ioc.factory.support.event.ObjectFactoryEvent.OBJECT_WIRED
	 */
	[Event(name = "objectWired", type = "org.springextensions.actionscript.ioc.factory.support.event.ObjectFactoryEvent")]
	/**
	...
	public class AbstractObjectFactory extends EventDispatcher implements IConfigurableObjectFactory, IAutowireProcessorAware, IDisposable {
		...
		private static var logger:ILogger = getLogger(AbstractObjectFactory);
		...
		protected function init():void {
			_eventBus = new org.as3commons.eventbus.impl.EventBus();

			_eventBusAdapter = new EventBusAdapter(_eventBus);
			...
			_objectPostProcessors = [new ObjectFactoryAwarePostProcessor(this), new ApplicationDomainAwarePostProcessor(this), new EventBusAwareObjectPostProcessor(this)];
		...
		protected function setIsReady(value:Boolean):void {
			_isReady = value;
		}
		...
		private var _eventBusAdapter:EventBusAdapter;
		...
		protected function buildObject(name:String, constructorArguments:Array = null):* {
			...
			if (!objectDefinition) {
				if (_parent) {
					...
					objectDefinition = getObjectDefinitionFromParent(objectName, _parent);
		...
		/**
		 * This helper method searches in the parent context for an object definition.
		 * If the definition is not found the method is called recursively to run through several parent factories.
		 *
		 * @param objectName the name of the object which is to be looked up
		 * @param parent the current parent object in which is searched
		 *
		 * @return the found object or null
		 */
		private function getObjectDefinitionFromParent(objectName:String, parent:IObjectFactory):* {
		...
		public function wire(object:*, objectDefinition:IObjectDefinition = null, objectName:String = null):void {
			...
			var evt:ObjectFactoryEvent = new ObjectFactoryEvent(ObjectFactoryEvent.OBJECT_WIRED, object, objectName);
			dispatchEvent(evt);
			_eventBus.dispatchEvent(evt);
		}
